const ResolutionModel = require("../model/ResolutionModel");
...
module.exports.saveResolution = async (req, res) => {
  const { name } = req.body;
  ResolutionModel.create({ name }).then((data) => {
    console.log("ADDED");
    res.send(data);
  });
};
...
module.exports.deleteResolution = async (req, res) => {
  const { _id } = req.body;
  ResolutionModel.findByIdAndDelete(_id).then(() =>
    res.send("Deleted resolution")
  );
};
//EDIT
...
module.exports.editResolution = async (req, res) => {
  ...
  ResolutionModel.findByIdAndUpdate(_id, { name }).then(() =>
    res.send("Edited resolution")
  );
};
